...
public class UmsMenuController {
    ...
    @ApiOperation("返回所有菜单列表")
    @RequestMapping(value = "/treeList", method = RequestMethod.GET)
    @ResponseBody
    public CommonResult<List<UmsMenu>> treeList() {
        List<UmsMenu> list = menuService.treeList();
        return CommonResult.success(list);
    }
    @ApiOperation("树形结构返回所有菜单列表")
    @RequestMapping(value = "/treeList2/{roleId}", method = RequestMethod.GET)
    @ResponseBody
    public CommonResult<List<Node>> treeList2(@PathVariable Long roleId) {
        List<Node> list = menuService.treeList2(roleId);
        return CommonResult.success(list);
    }
